...
package body Minerva.Values is
   ...
   begin
      return (case Operator is
                 ...
                 when Op_Negate   => I (-Left),
                 when Op_Exponent => I (Left ** Right),
                 when Op_Multiply => I (Left * Right),
                 when Op_Divide   => (if Right = 0
                                      then raise Constraint_Error with
                                        "division by zero"
                                      else I (Left / Right)),
                 when Op_Add      => I (Left + Right),
                 when Op_Subtract => I (Left - Right),
   ...
   begin
      return (case Operator is
                 ...
                 when Op_Negate => F (-Left),
                 when Op_Exponent => F (Left ** Integer (Right)),
                 when Op_Multiply => F (Left * Right),
                 when Op_Divide   => (if Right = 0.0
                                      then raise Constraint_Error with
                                        "division by zero"
                                      else F (Left / Right)),
                 when Op_Add      => F (Left + Right),
                 when Op_Subtract => F (Left - Right),
                 when Op_LE       => B (Left <= Right),
                 when Op_LT       => B (Left < Right),
                 when Op_GE       => B (Left >= Right),
                 when Op_GT       => B (Left > Right),
